    ...
    try:
        with st.spinner(f"Adding new accession IDs into UC table {uc_table}", show_time=True):
            ...
            for acc in accessions:
                query = f"INSERT INTO {uc_table} (id, record_create_ts, record_update_ts) VALUES ('{acc}', current_timestamp(), current_timestamp())"
...
# Add fasta batch into UC table raw.protein
def add_fasta_batch_uc_table(seq_list):
    uc_table = "workspace.raw.protein"
    try:
        with st.spinner(f"Adding new sequences into UC table {uc_table}", show_time=True):
            conn = dbh.get_databricks_connection()
            cursor = conn.cursor()
            cursor.execute(f"SELECT id FROM {uc_table}")
            existing_ids = set(row[0] for row in cursor.fetchall())
...
def get_seq_to_blast():
    ...
    try:
        with st.spinner(f"Identifying sequences to BLAST from UC table {uc_table}"):
            conn = dbh.get_databricks_connection()
            cursor = conn.cursor()
            cursor.execute(f"SELECT id, fasta_sequence FROM {uc_table} WHERE id NOT IN (SELECT DISTINCT blast_of_id FROM {uc_table} WHERE blast_of_id IS NOT NULL) AND blast_of_id IS NULL")
            rows = cursor.fetchall()
            fasta_sequences = [(row[0], row[1]) for row in rows]
            st.success(f"Successfully identified {len(fasta_sequences)} sequences to BLAST from UC table {uc_table}")
    ...
    with st.spinner(f"Adding BLAST sequences into UC table {uc_table} for accession: {accession}", show_time=True):
        try:
            ...
            for seq in blasted_sequence:
                ...
                insert_query = f"INSERT INTO {uc_table} (id, fasta_sequence, blast_of_id, blast_top_hit_number, blast_percent_identity, record_create_ts, record_update_ts) VALUES (?, ?, ?, ?, ?, current_timestamp(), current_timestamp())"
                # cursor.execute(insert_query, seq)
                cursor.execute(insert_query, (seq[0],seq[1],seq[2],seq[3],seq[4],))
